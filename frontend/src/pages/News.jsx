      ...
      <section className="section">
        <div className="container">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxWidth: 800, margin: '0 auto' }}>
            {filtered.map(item => (
              <div key={item.id} className="card" style={{ padding: '1.75rem', borderLeft: `4px solid ${item.important ? 'var(--burgundy)' : 'var(--seafoam)'}` }}>
          ...
          {filtered.length === 0 && (
            <div className="loading" style={{ flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <span>No announcements in this category</span>
            </div>
          )}
        </div>
      </section>
